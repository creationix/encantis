...
function main() {
  ...
  switch (command) {
    case 'ast': {
      ...
      if (result.errors.length > 0) {
        for (const error of result.errors) {
          ...
          console.error(`${filePath}:${loc.start.line}:${loc.start.column}: ${error.shortMessage}`);
      ...
      // Output AST as JSON
      console.log(JSON.stringify(result.module, (_key, value) => {
    ...
    case 'check': {
      ...
      console.log(`${filePath}: OK (${result.module?.decls.length ?? 0} declarations)`);
      break;
    }

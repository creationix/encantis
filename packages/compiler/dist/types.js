...
export function pointer(pointee) {
    return { kind: 'pointer', pointee };
}
export function indexed(element, size = null, specifiers = []) {
    return { kind: 'indexed', element, size, specifiers };
}
// Convenience constructors
export function slice(element) {
    return indexed(element, null, []);
}
export function array(element, size) {
    return indexed(element, size, []);
}
export function comptimeIndexed(element) {
    return indexed(element, 'comptime', []);
}
export function nullterm(element, size = null, level = 1) {
    const specifiers = Array(level).fill({ kind: 'null' });
    return indexed(element, size, specifiers);
}
export function tuple(fields) {
    return { kind: 'tuple', fields };
}
export function func(params, returns) {
    return { kind: 'func', params, returns };
}
export const VOID = { kind: 'void' };
export function comptimeInt(value) {
    return { kind: 'comptime_int', value };
}
export function comptimeFloat(value) {
    return { kind: 'comptime_float', value };
}
export function comptimeList(elements) {
    return { kind: 'comptime_list', elements };
}
export function named(name, type, unique) {
...
export function defaultIndexedType(t) {
    const LEB128_SPEC = { kind: 'prefix' };
...
function findInnermostElement(t) {
    if (t.kind === 'comptime_list') {
...
// === Type Equality ===
function specifierEquals(a, b) {
    return a.kind === b.kind;
}
function specifiersEqual(a, b) {
...
function specifiersCompatible(target, source) {
    if (target.length === 0)
        return true; // slice accepts anything
    ...
    switch (a.kind) {
        ...
        case 'func': {
            ...
            if (a.params.length !== bFunc.params.length)
                return false;
            if (a.returns.length !== bFunc.returns.length)
                return false;
            return (a.params.every((p, i) => fieldEquals(p, bFunc.params[i])) &&
...
const lossless = (reinterpret) => ({ compatible: true, lossiness: 'lossless', reinterpret });
const lossy = (reinterpret) => ({ compatible: true, lossiness: 'lossy', reinterpret });
// Check if source type can be assigned to target type
// Returns lossiness and reinterpretability
export function typeAssignResult(target, source) {
    ...
    if (t.kind === 'tuple' && s.kind === 'tuple') {
        if (t.fields.length !== s.fields.length)
            return INCOMPATIBLE;
        ...
        for (let i = 0; i < t.fields.length; i++) {
            ...
            if (!fieldResult.compatible || fieldResult.lossiness !== 'lossless')
                return INCOMPATIBLE;
            if (!fieldResult.reinterpret)
                allReinterpret = false;
        }
    ...
    if (t.kind === 'func' && s.kind === 'func') {
        ...
        if (t.params.length !== s.params.length)
            return INCOMPATIBLE;
        if (t.returns.length !== s.returns.length)
            return INCOMPATIBLE;
        // Check each param type matches exactly
        for (let i = 0; i < t.params.length; i++) {
            if (!typeEquals(t.params[i].type, s.params[i].type))
                return INCOMPATIBLE;
        }
        // Check each return type matches exactly
        for (let i = 0; i < t.returns.length; i++) {
            if (!typeEquals(t.returns[i].type, s.returns[i].type))
                return INCOMPATIBLE;
        }
...
export function typeAssignable(target, source) {
...
function specifierToSentinel(s) {
    return s.kind === 'null' ? '0' : '?';
}
export function typeToString(t, opts) {
    ...
    switch (t.kind) {
        ...
        case 'func': {
            ...
            if (t.returns.length === 0)
                return compact ? `${params}->()` : `func${params}`;
            const returns = t.returns.length === 1 && t.returns[0].name === null
...
export function isSigned(t) {
    const u = unwrap(t);
    return u.kind === 'primitive' && SIGNED.includes(u.name);
}
export function isUnsigned(t) {
    const u = unwrap(t);
...
export function primitiveByteSize(t) {
    const u = unwrap(t);
    if (u.kind !== 'primitive')
        return null;
    switch (u.name) {
...
export function isWideningConversion(from, to) {
...
export function isNarrowingConversion(from, to) {
    if (from === to)
        return false;
...
export function isFloatIntConversion(from, to) {
...
export function comptimeIntFits(value, target) {

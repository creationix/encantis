...
class MetaBuilder {
    ...
    symbols = [];
    symbolIndexByName = new Map();
    hints = {};
    constructor(module, source, lineMap, comments, checkResult) {
        this.module = module;
        this.source = source;
    ...
    findImportFuncOffset(item, name) {
    ...
    findKeywordIdentOffset(start, end, keyword) {
    ...
    collectFuncBody(decl, _funcSymbolIndex) {
                ...
                for (const param of decl.signature.input.fields) {
                    if (param.ident) {
                        ...
                        if (paramSymbolIndex !== undefined) {
                            this.addHint(param.span.start, param.ident.length, this.symbols[paramSymbolIndex].type, paramSymbolIndex);
                        }
                    }
                }
            }
    ...
    generateHintsForBlock(block) {
        for (const stmt of block.stmts) {
            this.generateHintsForStmt(stmt);
        }
    }
    generateHintsForStmt(stmt) {
        switch (stmt.kind) {
            case 'LetStmt':
                this.generateHintsForPattern(stmt.pattern);
                if (stmt.type)
                    this.generateHintsForTypeExpr(stmt.type);
                if (stmt.value)
            ...
            case 'ReturnStmt':
                if (stmt.value)
                    this.generateHintsForExpr(stmt.value);
                if (stmt.when)
                    this.generateHintsForExpr(stmt.when);
                break;
            case 'AssignmentStmt':
                this.generateHintsForExpr(stmt.value);
                break;
            case 'WhileStmt':
                ...
                if (stmt.body.kind === 'Block')
                    this.generateHintsForBlock(stmt.body);
                break;
            case 'LoopStmt':
                if (stmt.body.kind === 'Block')
                    this.generateHintsForBlock(stmt.body);
    ...
    generateHintsForPattern(pattern) {
        if (pattern.kind === 'IdentPattern') {
            const symbolIndex = this.symbolIndexByName.get(pattern.name);
            if (symbolIndex !== undefined) {
                this.addHint(pattern.span.start, pattern.name.length, this.symbols[symbolIndex].type, symbolIndex);
            }
        }
        // TODO: Handle TuplePattern
    }
    generateHintsForExpr(expr) {
        ...
        switch (expr.kind) {
            ...
            case 'CallExpr':
                ...
                for (const arg of expr.args) {
                    if (arg.value)
                        this.generateHintsForExpr(arg.value);
                }
                break;
            case 'BinaryExpr':
    ...
    generateHintsForTypeExpr(type) {
        if (type.kind === 'TypeRef') {
            const symbolIndex = this.symbolIndexByName.get(type.name);
            if (symbolIndex !== undefined) {
                this.addHint(type.span.start, type.name.length, this.symbols[symbolIndex].type, symbolIndex);
            }
        }
    ...
    generateHintsForTypeDecl(decl) {
        const symbolIndex = this.symbolIndexByName.get(decl.ident.name);
        if (symbolIndex !== undefined) {
            this.addHint(decl.ident.span.start, decl.ident.name.length, this.symbols[symbolIndex].type, symbolIndex);
        }
    }
    generateHintsForDef(decl) {
        const symbolIndex = this.symbolIndexByName.get(decl.ident);
        if (symbolIndex !== undefined) {
            const offset = this.findDefIdentOffset(decl);
            this.addHint(offset, decl.ident.length, this.symbols[symbolIndex].type, symbolIndex);
    ...
    generateHintsForGlobal(decl) {
        ...
        if (decl.type)
            this.generateHintsForTypeExpr(decl.type);
        if (decl.value)
            this.generateHintsForExpr(decl.value);
    }
    addHint(offset, len, typeIndex, symbolIndex) {
        const key = this.lineMap.positionKey(offset);
        const hint = { len, type: typeIndex };
        if (symbolIndex !== undefined)
            hint.symbol = symbolIndex;
        this.hints[key] = hint;
    }
}
// === Type Registry ===
class TypeRegistry {
    ...
    registerWithSymbol(type, symbolIndex) {
        const key = typeToString(type, { compact: true });
        const existing = this.typeIndex.get(key);
        if (existing !== undefined) {

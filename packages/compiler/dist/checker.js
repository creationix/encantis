...
import { primitive, pointer, indexed, slice, array, tuple, func, field, VOID, comptimeInt, comptimeFloat, comptimeList, comptimeIndexed, named, typeToString, comptimeIntFits, typeAssignable, defaultIndexedType, unwrap, } from './types';
// === Type Key Helper ===
/**
 ...
 */
export function typeKey(offset, kind) {
    return `${offset}:${kind}`;
}
 ...
 */
export function typecheck(module, options) {
 ...
 */
export function concretizeType(t, options = DEFAULT_OPTIONS) {
    const opts = { ...DEFAULT_OPTIONS, ...options };
    const u = unwrap(t);
    switch (u.kind) {
        case 'comptime_int':
 ...
 */
export function isConcreteType(t) {
    const u = unwrap(t);
    switch (u.kind) {
        case 'comptime_int':
        case 'comptime_float':
        ...
        case 'func':
            return (u.params.every((p) => isConcreteType(p.type)) &&
...
class CheckContext {
    types = new Map();
    errors = [];
    moduleScope = { parent: null, symbols: new Map() };
    currentScope = this.moduleScope;
    // Cache for resolved type aliases
    typeCache = new Map();
    // Reference tracking
    references = new Map(); // defOffset → refOffsets
    symbolRefs = new Map(); // usageOffset → defOffset
    symbolDefOffsets = new Map(); // name → defOffset
    ...
    checkFuncBody(decl) {
        ...
        const funcScope = { parent: this.moduleScope, symbols: new Map() };
    ...
    resolveType(type) {
        switch (type.kind) {
            ...
            case 'IndexedType': {
                ...
                const specifiers = type.specifiers.map((s) => s.kind === 'null' ? { kind: 'null' } : { kind: 'prefix' });
            ...
            case 'CompositeType': {
                const fields = type.fields.map((f) => field(f.ident, this.resolveType(f.type)));
                return tuple(fields);
            }
            case 'BuiltinType': {
    ...
    checkExpr(expr, expected, errorContext) {
        ...
        if (!typeAssignable(expected, inferred)) {
            this.error(expr.span.start, `${prefix}cannot assign ${typeToString(inferred)} to ${typeToString(expected)}`);
        }
    ...
    resolveListToIndexed(list, expected) {
    ...
    concretizeToTarget(type, expected) {
    ...
    inferCall(expr) {
        ...
        if (calleeType.kind === 'func') {
            ...
            if (argCount !== calleeType.params.length) {
                this.error(expr.span.start, `expected ${calleeType.params.length} arguments, got ${argCount}`);
            }
            ...
            case 'field': {
                ...
                if (objType.kind === 'indexed') {
                    if (expr.member.name === 'ptr')
                        return pointer(objType.element);
    ...
    recordReference(name, refOffset) {

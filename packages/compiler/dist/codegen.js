...
import { isSigned, isFloat, unwrap, primitiveByteSize, } from './types';
import { dataToWat, buildDataSection } from './data';
function createContext(checkResult) {
    return {
...
export function typeToWasm(t) {
    ...
    switch (u.kind) {
        ...
        default:
            throw new Error(`Unknown type kind: ${u.kind}`);
    }
}
/**
...
// === Expression Codegen ===
export function exprToWat(expr, ctx) {
    switch (expr.kind) {
        case 'LiteralExpr':
...
function literalToWat(expr, ctx) {
...
function memberToWat(expr, ctx) {
    ...
    if (member.kind === 'type') {
        ...
        if (loadType.kind === 'primitive') {
            ...
            if (name === 'i8')
                return `(i32.load8_s ${ptr})`;
            if (name === 'u16')
                return `(i32.load16_u ${ptr})`;
            if (name === 'i16')
                return `(i32.load16_s ${ptr})`;
        }
        return `(${wt}.load ${ptr})`;
    }
    return exprToWat(expr.object, ctx);
}
function indexToWat(expr, ctx) {
    const base = exprToWat(expr.object, ctx);
...
function matchToWat(expr, ctx) {
    ...
    for (let i = expr.arms.length - 1; i >= 0; i--) {
        ...
        const body = arm.body.kind === 'Block' || arm.body.kind === 'ArrowBody'
        ...
        if (isWildcard) {
            ...
            const conditions = arm.patterns.map((p) => {
                if (p.kind === 'literal') {
                    const litValue = p.value.kind === 'int'
            ...
            // Combine with OR
            const cond = conditions.length === 1
                ? conditions[0]
...
// === Statement Codegen ===
export function stmtToWat(stmt, ctx) {
    switch (stmt.kind) {
        case 'LetStmt':
...
function letToWat(stmt, ctx) {
    if (!stmt.value)
        return ''; // Declaration only, no initialization
...
function lvalueToWat(target, ctx) {
...
function assignLvalue(target, value, ctx) {
    if (target.kind === 'IdentExpr') {
...
function continueToWat(stmt, ctx) {
    if (stmt.when) {
        const cond = exprToWat(stmt.when, ctx);
...
// === Function Codegen ===
export function funcToWat(decl, checkResult) {
...
function collectLocals(body, ctx, checkResult) {
    const locals = [];
...
// === Module Codegen ===
export function moduleToWat(module, checkResult) {
    const ctx = createContext(checkResult);
    const parts = ['(module'];
...
function hasMemoryImport(module) {
    for (const decl of module.decls) {
        if (decl.kind === 'ImportDecl') {
            for (const item of decl.items) {
                if (item.item.kind === 'ImportMemory')
                    return true;
            }
        }
    }
...
function importItemToWat(moduleName, item, _ctx) {
    const imp = item.item;
...
function exportToWat(decl, _ctx) {
    ...
    switch (type.kind) {
        ...
        case 'IndexedType':
            return {
                ...
                specifiers: type.specifiers.map((s) => s.kind === 'null' ? { kind: 'null' } : { kind: 'prefix' }),
            };
        case 'CompositeType':
            return {
                kind: 'tuple',
